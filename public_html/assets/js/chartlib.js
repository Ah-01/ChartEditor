...
function GetDataAndStyle() {
    ...
    $.getJSON("assets/data/data.json", function (data, error) {
        ParsedData = data;
        ChartData.resolve();
    })
    $.getJSON("assets/data/style.json", function (data, error) {
        ParsedStyle = data;
        ChartStyle.resolve();
    })
    var chartNeccessarydata = $.when(ChartData, ChartStyle).done(function () {
        ChartInitParams = {
            Data: ParsedData,
            Style: ParsedStyle
        }
...
BarChart.prototype.setActualChartingAreaStyle = function (that) {
    that.actualChartArea = d3.select('.main-chart').append("div").classed("actualChartPane", true).style({
...
BarChart.prototype.renderLegend = function () {
    var that = this;
    var legendContainer = d3.select('.main-chart').append("div")
...
function editData(that) {
    ...
    $(".series-add").on("click", function () {
        console.log('i am on add series')
        that.AddSeries(that);
    })
    $(".label-add").on("click", function () {
        console.log('i am on add labels')
        that.AddLabels(that);
    });
}
